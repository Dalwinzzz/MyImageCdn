...
```
https://cdn.example.com/<资源类型>/<分类>/<文件名>
```

> `cdn.example.com` 为占位域名，绑定 Cloudflare 自定义域名后替换为你自己的域名。

源站为本仓库的 raw 内容，Worker 负责按扩展名修正 `Content-Type`（HTML / CSS / JS / 字体 / 图片）、删除 raw 强加的 CSP/sandbox 安全头并做缓存。
...
```
https://cdn.example.com/images/blog/cover.png
https://cdn.example.com/libs/mermaid/mermaid.min.js
https://cdn.example.com/pages/demo/index.html
```
...
Worker 代码部署在 Cloudflare（不入库），复刻时只改顶部三个常量：

```js
const GH_USER = 'Dalwinzzz';
const GH_REPO = 'MyImageCdn';
